...
//  #define SW520D_VIBRATION_PIN 2    // SW2 - Vibration monitoring (connect to ESP32 Pin 2)
//  #define SW520D_EMERGENCY_PIN 4    // SW4 - Emergency stop signal (connect to ESP32 Pin 4)
//  // SW1 is wired in series with MOSFET gate (hardware emergency stop)
//  // SW3 is wired in series with main power (manual power switch)
...
//  constexpr unsigned long VIBRATION_COOLDOWN = 500;  // 500ms cooldown between detections
//  constexpr unsigned long VIBRATION_RESET_TIME = 5000; // Reset vibration count every 5 seconds
//  constexpr uint8_t MAX_VIBRATIONS = 5;              // Max vibrations before action
 
//  /* -------- MG996R Servo Configuration -------- */
//  const uint8_t SERVO_PINS[6] = {12, 13, 18, 19, 21, 22};
//  Servo servos[6];
 
//  // Servo power gate (optional but recommended for power savings)
//  #define SERVO_PWR_PIN 16          // GPIO pin to control servo power via MOSFET
//                                    // Leave undefined if no MOSFET power control
...
//    // Apply global speed multiplier AFTER clamping
//    int adjustedSpeed = (speed * globalSpeedMultiplier) / 100;
//    adjustedSpeed = constrain(adjustedSpeed, -100, 100);
   
//    // Apply minimum speed boost for low speeds (but not zero)
...
//    // Debug output with both original and adjusted speeds
//    if (motorDiagnostics && speed != 0) {
//      Serial.printf("Motor: %d%% -> %d%% (global: %d%%)\n", 
//                    speed, adjustedSpeed, globalSpeedMultiplier);
//    }
//  }
 
//  /* -------- Individual Motor Control Functions -------- */
//  void motorFrontLeft(int speed) {
//    setMotorSpeed(D0_LEFT, D1_LEFT, speed);
//    if (motorDiagnostics && speed != 0) {
//      Serial.printf("🔄 FL: %d%%\n", speed);
//    }
//  }
...
//    setMotorSpeed(D2_LEFT, D3_LEFT, speed);
//    if (motorDiagnostics && speed != 0) {
//      Serial.printf("🔄 RL: %d%%\n", speed);
//    }
//  }
...
//    setMotorSpeed(D0_RIGHT, D1_RIGHT, speed);
//    if (motorDiagnostics && speed != 0) {
//      Serial.printf("🔄 FR: %d%%\n", speed);
//    }
//  }
...
//    setMotorSpeed(D2_RIGHT, D3_RIGHT, speed);
//    if (motorDiagnostics && speed != 0) {
//      Serial.printf("🔄 RR: %d%%\n", speed);
//    }
//  }
...
//  /* -------- SW-520D Sensor Functions -------- */
 
//  void setupVibrationSensors() {
//    Serial.println("🌀 Initializing SW-520D vibration sensors...");
   
//    // Configure SW-520D pins as inputs with pull-up resistors
...
//    Serial.printf("✅ SW-520D sensors configured:\n");
//    Serial.printf("   Vibration Monitor: Pin %d\n", SW520D_VIBRATION_PIN);
//    Serial.printf("   Emergency Stop: Pin %d\n", SW520D_EMERGENCY_PIN);
//    Serial.println("   Hardware E-Stop: Wired to MOSFET gate");
//    Serial.println("   Power Switch: Wired to main power");
//  }
 
//  void checkVibrationSensors() {
//    unsigned long currentTime = millis();
   
//    // Reset vibration count periodically
//    if (currentTime - lastVibrationTime > VIBRATION_RESET_TIME) {
//      if (vibrationCount > 0) {
//        vibrationCount = 0;
//        if (motorDiagnostics) {
//          Serial.println("🔄 Vibration count reset");
//        }
//      }
//    }
   
//    // Check vibration monitoring sensor (SW2)
...
//        if (motorDiagnostics) {
//          Serial.printf("🚨 Vibration #%d detected! (Pin %d)\n", 
//                        vibrationCount, SW520D_VIBRATION_PIN);
//        }
       
//        // Progressive response to vibrations
//        if (vibrationCount >= MAX_VIBRATIONS) {
//          Serial.println("⚠ EXCESSIVE VIBRATION - Emergency protocol activated!");
//          stopWheels();
//          servoPowerOff();
//          emergencyStopActive = true;
         
//          // Reset speed to minimum safe value
//          globalSpeedMultiplier = 30;
         
//        } else if (vibrationCount >= 3) {
//          // Reduce speed on multiple vibrations
//          if (globalSpeedMultiplier > 40) {
//            globalSpeedMultiplier = max(30, globalSpeedMultiplier - 15);
//            if (motorDiagnostics) {
//              Serial.printf("🔄 Speed reduced to %d%% due to vibration\n", globalSpeedMultiplier);
//            }
//          }
//        }
//      }
...
//    // Check emergency stop sensor (SW4)
//    if (digitalRead(SW520D_EMERGENCY_PIN) == LOW) {
//      if (!emergencyStopActive) {
//        emergencyStopActive = true;
//        Serial.printf("🛑 EMERGENCY STOP activated by SW-520D (Pin %d)!\n", SW520D_EMERGENCY_PIN);
       
//        // Immediate shutdown
//        stopWheels();
//        servoPowerOff();
       
//        // Set all servos to current position to prevent movement
//        for (int i = 0; i < 6; i++) {
//          targetServoPos[i] = currentServoPos[i];
//        }
       
//        Serial.println("🚨 All systems halted - Manual reset required");
//      }
//    } else {
//      // Emergency stop released
//      if (emergencyStopActive) {
//        emergencyStopActive = false;
//        vibrationCount = 0; // Reset vibration count
//        Serial.println("✅ Emergency stop released - Systems ready");
//      }
//    }
//  }
...
//  void resetVibrationSystem() {
//    vibrationDetected = false;
//    emergencyStopActive = false;
//    vibrationCount = 0;
//    lastVibrationTime = 0;
//    Serial.println("🔄 Vibration monitoring system reset");
//  }
...
//  #ifdef SERVO_PWR_PIN
//    // Initialize servo power gate pin
//    pinMode(SERVO_PWR_PIN, OUTPUT);
//    digitalWrite(SERVO_PWR_PIN, LOW);   // Start with servos OFF
//    Serial.printf("🔌 Servo power gate configured on pin %d\n", SERVO_PWR_PIN);
//  #endif
 
//    // Turn on servo power for initialization
//    servoPowerOn();
...
//    // Initialize SW-520D sensors
//    setupVibrationSensors();
//  }
 
//  // Enhanced smooth servo movement with power management
//  void smoothWrite(uint8_t servoId, uint8_t targetAngle) {
//    if (servoId >= 6) return;
...
//    // Get current position (if servo is attached) or assume 90°
//    uint8_t currentPos = servos[servoId].attached() ? servos[servoId].read() : 90;
   
//    // Attach servo if not already attached
//    if (!servos[servoId].attached()) {
//      servos[servoId].attach(SERVO_PINS[servoId], SERVO_MIN_PULSE, SERVO_MAX_PULSE);
//      delay(50); // Small delay after attachment
//    }
   
//    // Smooth movement in 1° steps
//    int step = (targetAngle > currentPos) ? 1 : -1;
//    while (currentPos != targetAngle) {
//      currentPos += step;
//      servos[servoId].write(currentPos);
//      delay(10); // 10ms between steps for smooth movement
//    }
...
//    delay(250); // Allow servo to settle
   
//    // Detach servo to save power and reduce heating
//    servos[servoId].detach();
   
//    // Turn off servo power to save energy
//    servoPowerOff();
   
//    if (motorDiagnostics) {
//      Serial.printf("🤖 Servo %d smoothly moved to %d° and detached\n", servoId, targetAngle);
//    }
//  }
...
//    // Turn on servo power for pose movement
//    servoPowerOn();
   
//    // Attach all servos for coordinated movement
//    for (int i = 0; i < 6; i++) {
...
//    // Detach all servos after pose is reached to save power
//    for (int i = 0; i < 6; i++) {
//      servos[i].detach();
//    }
...
//    // Use the smooth write function with power management
//    smoothWrite(servoId, angle);
   
//    if (motorDiagnostics) {
//      Serial.printf("🎯 Servo %d smoothly set to %d°\n", servoId, angle);
//    }
//  }
...
//      // Print SW-520D sensor status
//      port.printf(", Vibrations=%lu, EmergencyStop=%s", 
//                  vibrationCount, 
//                  emergencyStopActive ? "ACTIVE" : "OK");
//      port.println();
//    }
 
//    else if (cmd[0] == 'R') {                         // R - Reset vibration system
//      resetVibrationSystem();
//      port.println("OK R - Vibration system reset");
//      if (motorDiagnostics) Serial.println("🔄 Vibration system manually reset");
//    }
...
//        if (emergencyStopActive) {
//          port.println("ERR T: Emergency stop active - movement disabled");
//          return;
//        }
...
//  #ifdef SERVO_PWR_PIN
//    Serial.printf("   Servo Power Gate: Pin %d (MOSFET control)\n", SERVO_PWR_PIN);
//  #else
//    Serial.println("   Servo Power Gate: Not configured");
//  #endif
...
//    Serial.println("   V - Get detailed status (includes vibration data)");
//    Serial.println("   H - Home position");
//    Serial.println("   E - Emergency stop");
//    Serial.println("   R - Reset vibration monitoring system");
...
//    Serial.println("   V - Check vibration sensor status");
//    Serial.println("   R - Reset vibration system");
   
//    Serial.println("🌀 SW-520D Sensor Features:");
//    Serial.println("   • Automatic vibration detection and speed reduction");
//    Serial.println("   • Emergency stop on excessive vibration");
//    Serial.println("   • Hardware emergency stop (SW1 → MOSFET gate)");
//    Serial.println("   • Manual power control (SW3 → main power)");
   
//    Serial.println("🎮 Ready for commands!\n");
//  }
 
//  /* -------- Main Loop with Enhanced Safety and SW-520D Integration -------- */
//  void loop() {
//    // Process incoming commands
//    feed(Serial);
//    feed(BT);
   
//    // Check SW-520D vibration sensors (critical for safety)
//    checkVibrationSensors();
   
//    // Only allow servo updates if emergency stop is not active
//    if (!emergencyStopActive) {
